...
export default function Home() {
  ...
  return <main>
    ...
    <section id="contact" className="section-shell content-section contact-section"><Reveal><div className="contact-panel"><div className="contact-copy"><p className="eyebrow">03 / Hablemos</p><h2>¿Tienes un problema<br /><span>que vale la pena resolver?</span></h2><p>Cuéntame qué estás construyendo. Estoy disponible para contratación B2B y proyectos internacionales.</p><div className="social-links"><a href="https://github.com/alexbk09" target="_blank" rel="noreferrer" aria-label="GitHub"><Code2 /></a><a href="https://linkedin.com/in/keiber-paez-fullstack" target="_blank" rel="noreferrer" aria-label="LinkedIn"><Globe2 /></a><a href="mailto:alexbk09@hotmail.com" aria-label="Correo electrónico"><Mail /></a></div></div><form className="contact-form" onSubmit={handleSubmit}>{submitted ? <div className="success-state"><div className="success-icon"><Check /></div><h3>Mensaje recibido.</h3><p>Gracias por escribir. Este formulario está preparado para conectarse con Supabase.</p><button type="button" className="text-link" onClick={() => setSubmitted(false)}>Enviar otro mensaje</button></div> : <><label>Nombre<input required name="name" placeholder="Tu nombre" /></label><label>Correo electrónico<input required type="email" name="email" placeholder="tu@empresa.com" /></label><label>Mensaje<textarea required name="message" placeholder="¿En qué estás trabajando?" rows={4} /></label><button className="button button-primary" type="submit">Enviar mensaje <Send size={16} /></button></>}</form></div></Reveal></section>
